...
|------------------|---------|
|include/fpga      |Directory containing all header files|
|include/fgpa/fpga.h |Top-level header for user code to include|
|include/fpga/access.h |Header file for accelerator acquire/release, MMIO, memory management, event handling, etc. |
|include/fpga/bitstream.h |Header file for bitstream manipulation functions |
|include/fpga/common.h |Header file for error reporting functions |
|include/fpga/enum.h |Header file for AFU enumeration functions |
|include/fpga/manage.h |Header file for FPGA management functions |
|include/fpga/types.h |Various type definitions |
...
* `fpga_token`: An opaque type to represent a resource queried
but *not* owned by the calling process. The calling process must own a
...
`FPGA_EVENT_INTERRUPT`, `FPGA_EVENT_PORT_ERROR`, or `FPGA_EVENT_FME_ERROR`.
...
Then, passes it to `fpgaEnumerate()` to search for matching resources.
