...
```<target>``` can be either ```fpga``` or ```ase```.
...
header suppressed.
```console
./fpgadiag --mode=lpbk1 --target=fpga  --suppress-hdr --csv --bus=0x5e --begin=57535
...
```console
./fpgadiag --mode=read --target=fpga --bus=0xbe --begin=2045 --cont
...
```console
./fpgadiag --mode=sw --target=fpga  --bus=0xbe --begin=4 --end=8192
...
| Date |  Changes Made |
|:------|:--------------|
|2019.05.13| Made the following changes: <br> Corrected `bus-number=` argument in examples. This argument should be `bus=`. <br> Removed the `-S` shortcut for suppress header. <br>| 
